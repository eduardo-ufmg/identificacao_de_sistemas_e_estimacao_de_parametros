...
class OdometryDynamicModel:
    ...
    def simulate(self, odo_diff: np.ndarray, true_states: np.ndarray | None = None) -> np.ndarray:
        """
...
def load_model(model_json_path: str) -> tuple:
    ...
    if 'model_type' in data and data['model_type'] == 'narx':
        ...
        raise ValueError("Only NARX models are supported. Please use --narx when identifying the model.")
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=['one-step-ahead', 'free-run'], default='free-run',
        help="Simulation mode"
    )
    parser.add_argument(
        "--reference", default=None, help="Path to reference trajectory CSV (for one-step-ahead mode)"
    )
    ...
    if true_states is not None:
        ax.plot(true_states[:, 0], true_states[:, 1], 'k-', alpha=0.3, label="Reference")
