...
\begin{figure}[htbp]
    \centerline{\includegraphics[width=\linewidth]{odo_model_freerun.png}}
...
\begin{figure}[htbp]
    \centerline{\includegraphics[width=\linewidth]{filter_comparison.png}}
...
Future research should focus on enhancing the robustness of the filtering methods by integrating additional sensor modalities and improving the underlying models. Exploring advanced techniques such as deep learning for feature extraction and state estimation could provide significant improvements in localization accuracy. Additionally, implementing real-time loop closure detection and multi-hypothesis tracking may help mitigate the effects of drift and measurement ambiguity, leading to more reliable pose estimates in complex environments.

\subsubsection{Final Remarks}

In conclusion, while the particle filtering approach demonstrated superior performance in terms of error reduction, the overall results highlight the challenges inherent in robot pose estimation. The interplay between sensor noise, model fidelity, and filtering strategy remains a critical area of exploration. Continued advancements in sensor technology and algorithmic approaches will be essential for achieving the desired accuracy and reliability in robotic navigation tasks.
