classdef obstaclesImg < handle
    ...
    methods
        ...
        function obstaclePoints(this,decimationFactor,max_dist)
            ...
            while (nnz((visited(this.map == 0) == 0)) > 0)
                ...
                wave_front(i,j) = 1; frontCounter = 1; movesleft = true;
                while movesleft
                    movesleft = false;

                    [i,j] = find(wave_front == frontCounter);
                            ...
                            if ((i(ell) + neighbors_i(k))>0)&&((j(ell) + neighbors_j(k)) > 0)&&((i(ell) + neighbors_i(k))<siz_map(1))&&((j(ell) + neighbors_j(k))<siz_map(2))
                                if this.map(i(ell) + neighbors_i(k),j(ell) + neighbors_j(k)) == 0
                                    if visited(i(ell) + neighbors_i(k),j(ell) + neighbors_j(k)) == 0
                                        if (wave_front(i(ell) + neighbors_i(k),j(ell) + neighbors_j(k)) == 0)||(wave_front(i(ell) + neighbors_i(k),j(ell) + neighbors_j(k)) > frontCounter)
                                            wave_front(i(ell) + neighbors_i(k),j(ell) + neighbors_j(k)) = frontCounter + 1;
                                        end
            ...
            this.obstacles = [xm(1:decimationFactor:end), ym(1:decimationFactor:end)];
            ...
            for i = 1:numObstacles
                ...
                [ym, xm] = px2mts(this, xp, yp);
                this.points{i} = [xm(1:decimationFactor:end),ym(1:decimationFactor:end)];
