...
def fit_narx_model(ref: np.ndarray, odo_diff: np.ndarray, step: int = 1, n_lags: int = 3,
...
def grid_search_narx(ref: np.ndarray, odo_diff: np.ndarray, 
                     n_lags_list: list, model_types: list,
                     poly_degrees: list, alphas: list, 
                     hidden_layers_list: list, cv_splits: int = 5):
    """
        ...
        try:
            ...
            for train_idx, val_idx in tscv.split(states):
                ...
                model, poly_features, _, config = fit_narx_model(
                    ...
                    hidden_layers=tuple(params['hidden_layers']) if params['hidden_layers'] else (50, 50),
                    alpha=params['alpha']
                )
...
def main():
    ...
    parser.add_argument(
        "--reference", default="ref_dec_trimmed.csv", help="Path to reference trajectory CSV"
    )
    parser.add_argument(
        "--odo-diff", default="odo_dec_diff_trimmed.csv", help="Path to odometry differences CSV"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--poly-degree", type=int, default=2, help="Polynomial degree (for polynomial NARX)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-model", default=None, help="Path prefix to save model files (.json and .pkl)"
    )
    ...
    parser.add_argument(
        "--grid-n-lags", type=int, nargs='+', default=[1, 2, 3, 5],
        help="List of n_lags values for grid search"
    )
    parser.add_argument(
        "--grid-model-types", choices=['linear', 'polynomial', 'neural'], nargs='+',
        default=['linear', 'polynomial', 'neural'], help="Model types for grid search"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--grid-alphas", type=float, nargs='+', default=[0.001, 0.01, 0.1, 1.0],
        help="Alpha values for grid search"
    )
    parser.add_argument(
        "--grid-hidden-layers", type=str, nargs='+', default=['50,50', '100,50', '25,25,25'],
    ...
    if args.grid_search:
        ...
        if args.save_model and best_params:
            ...
            model, poly_features, rmse, config = fit_narx_model(
                ...
                hidden_layers=tuple(best_params['hidden_layers']) if best_params['hidden_layers'] else (50, 50),
        ...
        print("Example: python3 identify_odometry_dynamic_model.py --narx --save-model odo_narx")
        print("Example: python3 identify_odometry_dynamic_model.py --grid-search --save-model best_odo_narx")
