...
def main():
    ...
    print(f"N_particles bounds: {args.n_particles_bounds}")
    print(f"Q bounds: {args.q_bounds}")
    print(f"R bounds: {args.r_bounds}")
    print(f"Resample threshold bounds: {args.resample_bounds}")
    ...
    n_particles_opt = int(result.x[0])
    q_opt = result.x[1:4]
    r_opt = result.x[4:7]
    resample_threshold_opt = result.x[7]

    print(f"\nOptimization complete!")
    print(f"Total evaluations: {len(result.func_vals)}")
    print(f"Best objective value (RMSE): {result.fun:.6e}")
    print(f"Optimized N_particles: {n_particles_opt}")
    print(f"Optimized Q std: {q_opt}")
    print(f"Optimized R std: {r_opt}")
    print(f"Optimized resample threshold: {resample_threshold_opt:.4f}")

    # Save parameters
    params = {
        "n_particles": n_particles_opt,
        "q_std": [float(x) for x in q_opt],
        "r_std": [float(x) for x in r_opt],
        "resample_threshold": float(resample_threshold_opt),
        "objective_value": float(result.fun),
        "method": "Bayesian Optimization (GP)",
        "n_calls": args.n_calls,
        "n_initial_points": args.n_initial,
        "total_evaluations": len(result.func_vals),
        "n_particles_bounds": args.n_particles_bounds,
