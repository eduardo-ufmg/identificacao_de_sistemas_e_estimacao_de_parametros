...
def main():
	parser = argparse.ArgumentParser(description='Identify a linear dynamic model from reference and laser data')
	parser.add_argument('--ref', default='ref.csv', help='Path to reference trajectory CSV (x,y,theta)')
	parser.add_argument('--laser', default='laser.csv', help='Path to laser CSV (time x beams)')
	parser.add_argument('--step', type=int, default=1, help='Time step delta for the model (default: 1)')
	parser.add_argument('--save-model', default=None, help='Path to save model parameters (.npz)')
	...
	if args.save_model:
		np.savez(args.save_model + '.npz', A=A, B=B, bias=bias, rmse=rmse, coef=model.coef_, intercept=model.intercept_)
		...
		model_dict = {
			...
			'intercept': model.intercept_.tolist() if type(model.intercept_) is np.ndarray else model.intercept_,
		}
