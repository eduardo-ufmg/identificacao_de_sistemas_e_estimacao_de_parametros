...
class LaserDynamicModel:
	...
	def __init__(self, A: np.ndarray, B: np.ndarray, bias: np.ndarray, initial_pose: np.ndarray):
		"""
...
def main():
	parser = argparse.ArgumentParser(description='Simulate robot position from laser scans using fitted dynamic model')
	parser.add_argument('--laser', default='laser_dec.csv', help='Path to laser data CSV')
	parser.add_argument('--model', default='laser_model.json', help='Path to model parameters JSON')
