...
def load_data(args):
    ...
    if odo_model_params is not None:
        ...
        odo_model_obj = OdometryNARXModel(
            odo_narx_model, odo_poly_features, odo_narx_config, initial_pose
        )
...
def run_filter(
    filter_class, filter_name: str, data: dict, params: dict, verbose: bool = True
    ...
    if filter_name == "UKF":
        filter_obj = UKFEstimator(
            initial_pose, q_std=params["q_std"], r_std=params["r_std"]
        )
    ...
    trajectory = filter_obj.run(
        odo_deltas, laser_data, laser_model_params, odo_model_params
    )
...
def print_metrics_table(all_metrics: list):
    ...
    print("-" * 80)
    print(
        f"{'Filter':<10} {'RMSE':>10} {'Mean':>10} {'Median':>10} {'Std':>10} {'Max':>10} {'95%ile':>10}"
    )
    print("-" * 80)
    for m in all_metrics:
        print(
            f"{m['filter']:<10} {m['pos_rmse']:>10.4f} {m['pos_mean']:>10.4f} "
    ...
    print("-" * 80)
    print(
        f"{'Filter':<10} {'RMSE':>10} {'Mean':>10} {'Median':>10} {'Std':>10} {'Max':>10}"
    )
    print("-" * 80)
    for m in all_metrics:
        print(
            f"{m['filter']:<10} {m['theta_rmse']:>10.2f} {m['theta_mean']:>10.2f} "
...
def statistical_tests(trajectories: dict, reference: np.ndarray):
    ...
    for i, filter1 in enumerate(filters):
        for filter2 in filters[i + 1 :]:
            ...
            print(
                f"{filter1:>6} vs {filter2:<6}: p-value = {p_value:.4e} {significance}"
            )
    ...
    statistic, p_value = stats.friedmanchisquare(
        *[error_matrix[:, i] for i in range(len(filters))]
    )
    print(f"Chi-square statistic: {statistic:.4f}")
    print(f"p-value: {p_value:.4e}")
    if p_value < 0.05:
        print(
            "Conclusion: At least one filter performs significantly differently from others"
        )
    else:
...
def plot_comparison(data: dict, trajectories: dict, exec_times: dict, output_path: str):
    ...
    arrow_spacing = max(
        1, len(data["odo_deltas"]) // 20
    )  # Show ~20 arrows per trajectory
    ...
    ax_main.set_title(
        "Filter Comparison: Trajectory Estimates", fontsize=14, fontweight="bold"
    )
    ...
    if ref_traj is not None:
        ...
        bars = ax_time.bar(
            names, times, color=[colors[name] for name in names], alpha=0.7
        )
    ...
    info_text += (
        f"Reference: {'Available' if ref_traj is not None else 'Not available'}"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--show-plot", action="store_true", help="Show plot interactively after saving"
    )
    parser.add_argument(
...
def main():
    ...
    if is_narx:
        _, _, _, narx_config = data["laser_model_params"]
        print(
            f"Using laser NARX model: {narx_config['model_type']}, n_lags={narx_config['n_lags']}"
        )
    else:
